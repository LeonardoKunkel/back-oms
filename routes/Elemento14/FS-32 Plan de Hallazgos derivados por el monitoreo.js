const express = require('express'),
      HallazgoDerivadoMonitoreo = require('../models/Elemento 14/FS-31 Evaluación de Requisitos Legales/FS-32 Plan de Hallazgos derivados por el monitoreo'),
...
router.post('/create',[verificarToken], (req,res) => {
    ...
    HallazgoDerivadoMonitoreo.create(hallazgoDerivado,(err, hallazgoDerivadoBd) => {
        if (err) {
            res.status(400).json({
...
router.get('/', [verificarToken], (req,res) => {
    HallazgoDerivadoMonitoreo.find().
        exec((err,hallazgoDerivado) => {
            if (err) {
                res.status(400).json({
